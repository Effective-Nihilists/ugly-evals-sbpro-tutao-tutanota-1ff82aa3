...
exports.encodeNumbers = exports.decodeNumbers = exports.numberOfBytes = exports.calculateNeededSpaceForNumbers = exports.calculateNeededSpaceForNumber = exports.calculateNeededSpace = exports.decodeNumberBlock = exports.decodeBinaryBlock = exports.encodeNumberBlock = exports.encodeBinaryBlock = exports.appendBinaryBlocks = exports.removeBinaryBlockRanges = exports.iterateBinaryBlocks = void 0;
/**
...
function iterateBinaryBlocks(row, cb) {
    ...
    while (offset < row.length) {
        var block = decodeBinaryBlock(row, offset);
        var start = offset;
...
exports.iterateBinaryBlocks = iterateBinaryBlocks;
/**
...
exports.removeBinaryBlockRanges = removeBinaryBlockRanges;
/**
...
function appendBinaryBlocks(source, destination) {
    var neededSpace = calculateNeededSpace(source);
    var target;
    var offset;
    if (destination) {
...
exports.appendBinaryBlocks = appendBinaryBlocks;
/**
...
function encodeBinaryBlock(entityData, target, offset) {
    ...
    var lengthOfPrefix = encodeNumberBlock(entityData.length, target, offset);
    target.set(entityData, offset + lengthOfPrefix);
    return lengthOfPrefix + entityData.length;
}
exports.encodeBinaryBlock = encodeBinaryBlock;
/**
...
function encodeNumberBlock(value, target, offset) {
    ...
    if (value <= 0x7f) {
        ...
        var length_1 = numberOfBytes(value);
        var remainingValue = value;
        // Set highest bit to 1
        target[offset] = length_1 | 0x80;
        for (var i = 0; i < length_1; i++) {
            var bytePosition = offset + length_1 - i;
...
exports.encodeNumberBlock = encodeNumberBlock;
/**
...
function decodeBinaryBlock(source, offset) {
    var blockLength = decodeNumberBlock(source, offset);
    var numberLength = calculateNeededSpaceForNumber(blockLength);
    return source.subarray(offset + numberLength, offset + numberLength + blockLength);
}
exports.decodeBinaryBlock = decodeBinaryBlock;
/**
...
function decodeNumberBlock(source, offset) {
    ...
    // a short one.
    var markerBit = source[offset] & 0x80;
    if (markerBit === 0x80) {
        // Clear the first bit to get the length of number
        var numberLength = source[offset] & 0x7f;
        var value = 0;
...
exports.decodeNumberBlock = decodeNumberBlock;
/**
...
function calculateNeededSpace(data) {
    return data.reduce(function (acc, entry) {
        ...
        var lengthOfPrefix = calculateNeededSpaceForNumber(entry.length);
        return acc + entry.length + lengthOfPrefix;
    }, 0);
}
exports.calculateNeededSpace = calculateNeededSpace;
/**
...
exports.calculateNeededSpaceForNumber = calculateNeededSpaceForNumber;
function calculateNeededSpaceForNumbers(numbers) {
    return numbers.reduce(function (acc, n) { return acc + calculateNeededSpaceForNumber(n); }, 0);
}
exports.calculateNeededSpaceForNumbers = calculateNeededSpaceForNumbers;
/**
 ...
 */
function decodeNumbers(binaryNumbers, offset) {
    if (offset === void 0) { offset = 0; }
    var numbers = [];
    while (offset < binaryNumbers.length) {
        var number = decodeNumberBlock(binaryNumbers, offset);
        numbers.push(number);
        offset += calculateNeededSpaceForNumber(number);
    }
    return numbers;
}
exports.decodeNumbers = decodeNumbers;
/**
 ...
 */
function encodeNumbers(numbers, target, offset) {
    if (offset === void 0) { offset = 0; }
    numbers.forEach(function (number) {
        offset += encodeNumberBlock(number, target, offset);
    });
}
exports.encodeNumbers = encodeNumbers;
