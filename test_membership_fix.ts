...
/**
 * Verification test for the membership loss bug fix.
 * 
 * This test verifies that when a user loses membership in a group,
 * the corresponding entry in lastUpdateBatchIdPerGroup is properly deleted.
 */

import {EphemeralCacheStorage} from "./src/api/worker/rest/EphemeralCacheStorage"
import {DefaultEntityRestCache} from "./src/api/worker/rest/DefaultEntityRestCache"
import {createUser, createGroupMembership, GroupType, createEntityUpdate, OperationType, UserTypeRef} from "./src/api/entities/sys/TypeRefs"
...
async function testMembershipLossClearsBatchId() {
	...
	const restClient: EntityRestClient = {
